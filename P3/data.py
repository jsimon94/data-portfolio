...
import schema
from audit import is_street_name, is_zipcode, is_phone
OSM_PATH = "sample.osm"
...
def update_name(name, mapping):
    '''Cleans street name for insertion into database.'''
...
def update_postcode(postcode):
    '''Cleans postcodes for insertion into SQL Database'''
    search = re.match(r'^\D*(\d{5}).*', postcode)
    #Searches for postcodes whose length is more than 5 and returns
    #5 digit postcodes.
    clean_postcode = search.group(1)
    return clean_postcode

#Phone Number is in the format (718) 123-4567
...
def shape_element(element, node_attr_fields=NODE_FIELDS, way_attr_fields=WAY_FIELDS,
    ...
    if element.tag == 'node':
        ...
        for child in element:
            ...
            #Clean phone numbers, zipcodes, and street names
            	if is_street_name(child):
                	data['value'] = update_name(child.attrib['v'], mapping)
            	if is_zipcode(child):
                	data['value'] = update_postcode(child.attrib['v'])
            	if is_phone(child):
